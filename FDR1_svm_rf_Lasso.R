...
y=as.factor(c(rep(0,18),rep(1,26), rep(0,20))) 
...
p.fdr1=0
for(i in 1:k1){
  p.fdr1[i]=msort[i,2]*nrow(m)/sum(l[1:i]) #.05#.015
}
k.FDR1=cumsum(p.fdr1<.0005)
k.fdr1=0
for (i in 1:nrow(m)){
  k.fdr1[i]= (ifelse (k.FDR1[i+1]==k.FDR1[i], i ,NA) )
}
k2=min(k.fdr1,na.rm=T);k2
k1=k2
xk1=x3[,msort[1:k1,1]]
dim(xk1)
...
######LASSO#############
library(glmnet)
cvfit=cv.glmnet(xk1,y3, family="binomial", alpha=1,nfolds=10 )
preds <- predict(cvfit, newx=as.matrix(xk1), s="lambda.min", type="response")
library(pROC)
...
nsim=1000
auc.lasso=auc.ens=0
b.lasso=matrix(0,nrow=ncol(xk1)+1,ncol=nsim)
...
xk1=x3[,msort[1:k1,1]]
library(glmnet)
cvfit=cv.glmnet(xk1,y3, family="binomial", alpha=0.5,nfolds=10 )
preds <- predict(cvfit, newx=as.matrix(xk1), s="lambda.min", type="response")
library(pROC)
...
nsim=1000
auc.elnet=auc.ens=0
b.elnet=matrix(0,nrow=ncol(xk1)+1,ncol=nsim)
...
mtry_value <- floor(sqrt(ncol(xk1)))

rf_model <- randomForest(x = xk1, 
                         y = factor(y3), 
                         ntree = 1000, 
                         importance = TRUE, 
                         mtry = mtry_value)
...
nsim=1000
auc.rf=auc.ens=0
importance.rf=matrix(0,nrow=ncol(xk1),ncol=nsim)
...
nsim=1000
auc.svm=auc.ens=0
importance.svm=matrix(0,nrow=ncol(xk1),ncol=nsim)
...
for(bs in 1:nsim){
  ...
  x.svm=data.frame(xk1[id.sample,])
...
print(combined)
ggsave("E://papers/MLS/PLOS one/Figs/Fig7.png", plot = combined, width = 8, height = 8, dpi = 600)
